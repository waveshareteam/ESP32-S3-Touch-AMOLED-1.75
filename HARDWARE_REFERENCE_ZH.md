...
| P3 / `EXIO3` | `RTC_INT` | 输入 | PCF85063 中断。 |
| P4 / `EXIO4` | `SYS_OUT` | 输入 | 整形后的 PWR 按键状态；按下为高电平，松开为低电平。 |
...
- PWR 按键属于 AXP2101 电源控制路径，并非空闲的 ESP32-S3 GPIO，但整形后的 `SYS_OUT` 状态可从 TCA9554 P4/`EXIO4` 读取：按下为高电平，松开为低电平；读取该电平不需要访问 AXP2101 寄存器。BOOT 按键连接 GPIO0，带外部上拉，按下时读取为低电平。
...
5. ES8311、ES7210、功放控制路径及共用时钟应交由 BSP 管理。直接切换 GPIO46 可能与 Codec 生命周期管理冲突。
