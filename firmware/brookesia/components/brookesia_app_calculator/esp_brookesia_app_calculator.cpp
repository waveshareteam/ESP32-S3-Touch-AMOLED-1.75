...
// A square inscribed in a circle is about 70.7% of the display diameter.
// Keep the complete keypad rectangle just inside that boundary.
#define ROUND_SAFE_PERCENT       70
...
static constexpr lv_style_selector_t KEYBOARD_ITEMS_DEFAULT =
...
Calculator *Calculator::requestInstance(bool use_status_bar, bool use_navigation_bar)
{
    if (_instance == nullptr) {
        _instance = new Calculator(use_status_bar, use_navigation_bar);
    }
    return _instance;
}
...
bool Calculator::run(void)
{
    ...
    const uint16_t visual_height = _height;
    const int dimension_delta = static_cast<int>(_width) - static_cast<int>(_height);
    // A fixed status bar reduces the app-local height, but the framebuffer is
    // still the same round panel. Treat a modest top crop as round as well.
    const bool is_round_screen = (_width >= 300 && _height >= 300 &&
                                  dimension_delta >= -8 && dimension_delta <= _width / 4);
    int y_offset = 0;
    if (is_round_screen) {
        const uint16_t diameter = _width;
        const uint16_t safe_size = diameter * ROUND_SAFE_PERCENT / 100;
        const int safe_top_global = (diameter - safe_size) / 2;
        const int visual_top_global = visual_area.y1;
        int safe_top_local = safe_top_global - visual_top_global;
        int safe_bottom_local = safe_top_global + safe_size - visual_top_global;
        safe_top_local = (safe_top_local > 0) ? safe_top_local : 0;
        safe_bottom_local = (safe_bottom_local < visual_height) ? safe_bottom_local : visual_height;

        _width = safe_size;
        _height = (safe_bottom_local > safe_top_local)
                  ? safe_bottom_local - safe_top_local : visual_height;
        y_offset = safe_top_local;
    ...
    // On a round panel the label and keypad share one centered, inscribed
    // rectangle. Every key remains visible and touchable, including the four
    // outer keys in the first and last rows.
    if (is_round_screen) {
        lv_obj_align(keyboard, LV_ALIGN_TOP_MID, 0, y_offset + label_h);
    } else {
    ...
    lv_obj_set_style_text_color(keyboard, KEYBOARD_SPECIAL_COLOR, KEYBOARD_ITEMS_DEFAULT);
...
void Calculator::keyboard_event_cb(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_VALUE_CHANGED) {
        ...
        if (lv_obj_get_style_text_font(app->formula_label, LV_PART_MAIN) == LABEL_FONT_SMALL) {
