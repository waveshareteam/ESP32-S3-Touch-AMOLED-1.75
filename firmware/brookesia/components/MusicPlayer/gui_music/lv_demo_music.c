...
void lv_demo_music(lv_obj_t *parent, file_iterator_instance_t *file_iterator)
{
    ...
    const size_t iterator_count = (_file_iterator != NULL) ?
                                  file_iterator_get_count(_file_iterator) : 0;
    active_track_cnt = (iterator_count > UINT32_MAX) ? UINT32_MAX : (uint32_t)iterator_count;
    original_screen_bg_color = lv_obj_get_style_bg_color(parent, LV_PART_MAIN);
...
void lv_demo_music_close(void)
{
    ...
    /* Object deletion removes only this demo's animations. Deleting every
     * LVGL animation here used to corrupt other Brookesia applications. */
    if(demo_parent && lv_obj_is_valid(demo_parent)) {
        lv_obj_clean(demo_parent);
        lv_obj_set_style_bg_color(demo_parent, original_screen_bg_color, 0);
    }
    demo_parent = NULL;
    ctrl = NULL;
    list = NULL;
