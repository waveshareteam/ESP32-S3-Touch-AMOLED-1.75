...
#define ESP_XIAOZHI_EVENT_POST_TIMEOUT pdMS_TO_TICKS(100)
...
static esp_err_t esp_xiaozhi_chat_goodbye_handler(esp_xiaozhi_chat_t *xiaozhi_chat, cJSON *root)
{
    ...
    if (session_id == NULL || strcmp(xiaozhi_chat->session_id, session_id->valuestring) == 0) {
        esp_event_post(ESP_XIAOZHI_CHAT_EVENTS, ESP_XIAOZHI_CHAT_EVENT_SERVER_GOODBYE, NULL, 0,
                       ESP_XIAOZHI_EVENT_POST_TIMEOUT);
    }
...
static esp_err_t esp_xiaozhi_chat_connected_handler(esp_xiaozhi_transport_handle_t handle)
{
    ...
    esp_event_post(ESP_XIAOZHI_CHAT_EVENTS, ESP_XIAOZHI_CHAT_EVENT_CONNECTED, NULL, 0,
                   ESP_XIAOZHI_EVENT_POST_TIMEOUT);
...
static esp_err_t esp_xiaozhi_chat_disconnected_handler(esp_xiaozhi_transport_handle_t handle)
{
    ...
    esp_event_post(ESP_XIAOZHI_CHAT_EVENTS, ESP_XIAOZHI_CHAT_EVENT_DISCONNECTED, NULL, 0,
                   ESP_XIAOZHI_EVENT_POST_TIMEOUT);
...
static esp_err_t esp_xiaozhi_chat_stop_runtime(esp_xiaozhi_chat_t *xiaozhi_chat)
{
    ...
    if (xiaozhi_chat->mcp_mgr_handle != 0) {
        ...
        if (step_ret != ESP_OK) {
            ...
            /* This failure is critical: the manager still owns live state. */
            ret = step_ret;
        } else {
...
static void esp_xiaozhi_chat_release_internal_resources(esp_xiaozhi_chat_t *xiaozhi_chat)
{
    if (xiaozhi_chat == NULL) {
        return;
    }
    ...
    if (xiaozhi_chat->udp != NULL) {
        ...
        if (xiaozhi_chat->udp->channel_mutex != NULL) {
            ...
            xiaozhi_chat->udp->channel_mutex = NULL;
        }
        free(xiaozhi_chat->udp);
        xiaozhi_chat->udp = NULL;
    }

    if (xiaozhi_chat->event_group_handle != NULL) {
        vEventGroupDelete(xiaozhi_chat->event_group_handle);
        xiaozhi_chat->event_group_handle = NULL;
    }
}
...
esp_err_t esp_xiaozhi_chat_init(esp_xiaozhi_chat_config_t *config, esp_xiaozhi_chat_handle_t *chat_hd)
{
    ...
    *chat_hd = 0;
    ...
    if (xiaozhi_chat->use_mqtt) {
        ...
        if (xiaozhi_chat->udp == NULL) {
            ESP_LOGE(TAG, "Failed to allocate UDP context");
            ret = ESP_ERR_NO_MEM;
            goto error;
        }
        ...
        if (xiaozhi_chat->udp->audio_receive_events == NULL) {
            ESP_LOGE(TAG, "Failed to create receive event group");
            ret = ESP_ERR_NO_MEM;
            goto error;
        }
        xEventGroupSetBits(xiaozhi_chat->udp->audio_receive_events, ESP_XIAOZHI_CHAT_RECV_TASK_EXITED);
    ...
    ret = esp_xiaozhi_chat_register_instance(xiaozhi_chat);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Only one chat instance is supported");
        goto error;
    }
...
error:
    /* Ownership of caller-provided MCP resources transfers only after init succeeds. */
    esp_xiaozhi_chat_unregister_instance(xiaozhi_chat);
    esp_xiaozhi_chat_release_internal_resources(xiaozhi_chat);
    free(xiaozhi_chat);
    return ret;
}
...
esp_err_t esp_xiaozhi_chat_deinit(esp_xiaozhi_chat_handle_t chat_hd)
{
    ...
    if (xiaozhi_chat->udp != NULL && xiaozhi_chat->udp->audio_receive_events != NULL) {
        ...
        if (!(bits & ESP_XIAOZHI_CHAT_RECV_TASK_EXITED)) {
            ESP_LOGE(TAG, "Timed out waiting for audio input task to exit; resources retained for deinit retry");
    ...
    if (xiaozhi_chat->mcp_mgr_handle != 0) {
        ESP_LOGE(TAG, "MCP manager teardown incomplete; resources retained for deinit retry");
        return (ret != ESP_OK) ? ret : ESP_FAIL;
    }
    if (ret != ESP_OK) {
        /* The manager is gone, so close/stop errors no longer block safe teardown. */
        ESP_LOGW(TAG, "Ignoring non-fatal runtime stop error after MCP manager teardown: %s", esp_err_to_name(ret));
    }

    esp_err_t destroy_ret = esp_xiaozhi_chat_destroy_owned_resources(xiaozhi_chat);
    if (destroy_ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to destroy owned MCP engine; resources retained for deinit retry: %s",
...
error:
    if (xiaozhi_chat->mcp_mgr_handle != 0) {
        esp_err_t cleanup_ret = esp_mcp_mgr_stop(xiaozhi_chat->mcp_mgr_handle);
        if (cleanup_ret != ESP_OK) {
            ESP_LOGW(TAG, "Failed to stop MCP manager after start error: %s", esp_err_to_name(cleanup_ret));
        }

        cleanup_ret = esp_mcp_mgr_deinit(xiaozhi_chat->mcp_mgr_handle);
        if (cleanup_ret == ESP_OK) {
            xiaozhi_chat->mcp_mgr_handle = 0;
            xiaozhi_chat->mcp_chat_handle = 0;
        } else {
            /* Preserve the manager handle so chat_deinit() can retry cleanup. */
            ESP_LOGE(TAG, "Failed to deinit MCP manager after start error: %s", esp_err_to_name(cleanup_ret));
        }
    }
                                              ...
                                              char *message, size_t message_len)
{
    ...
    esp_event_post(ESP_XIAOZHI_CHAT_EVENTS, ESP_XIAOZHI_CHAT_EVENT_AUDIO_CHANNEL_OPENED, NULL, 0,
                   ESP_XIAOZHI_EVENT_POST_TIMEOUT);
...
esp_err_t esp_xiaozhi_chat_close_audio_channel(esp_xiaozhi_chat_handle_t chat_hd)
{
    ...
    esp_event_post(ESP_XIAOZHI_CHAT_EVENTS, ESP_XIAOZHI_CHAT_EVENT_AUDIO_CHANNEL_CLOSED, NULL, 0,
                   ESP_XIAOZHI_EVENT_POST_TIMEOUT);
