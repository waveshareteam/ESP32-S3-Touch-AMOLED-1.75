...
typedef struct {
    ...
    bool owns_mcp_engine;            /*!< Whether chat takes ownership of mcp_engine after successful init and destroys it in deinit */
 ...
 * `config.owns_mcp_engine` was set to true during init. If the receive task does not exit
 * within the shutdown timeout, its socket is closed to wake it, but no backing context,
 * synchronization object, or engine is destroyed. More generally, for any teardown
 * error returned for a valid handle, the handle remains owned by the caller and must be
 * passed to this function again. Only ESP_OK means the handle has been released.
 *
 ...
 *       - ESP_ERR_TIMEOUT       Receive task is still exiting; backing resources and handle are retained for a deinit retry
 *       - Other                 Runtime or owned-resource teardown failed; handle is retained for a deinit retry
 */
